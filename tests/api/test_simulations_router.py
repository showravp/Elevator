...
def test_submit_requests_rejects_negative_request_fields(tmp_path: Path) -> None:
    ...
    response = client.post(f"/simulations/{run_id}/requests", json=requests)
...
def test_two_concurrent_runs_stay_isolated(tmp_path: Path) -> None:
    ...
    client.post(
        f"/simulations/{run_a}/requests",
        json={"requests": [{"time": 0, "id": "a1", "source": 0, "dest": 4}]},
    )
    client.post(
        f"/simulations/{run_b}/requests",
        json={
            "requests": [
...
def test_update_config_while_pending_succeeds(tmp_path: Path) -> None:
    ...
    response = client.put(f"/simulations/{run_id}/config", json=new_config)
...
def test_update_config_after_requests_submitted_returns_409(tmp_path: Path) -> None:
    ...
    run_id = _create_run(client)
    client.post(
        f"/simulations/{run_id}/requests",
        json={"requests": [{"time": 0, "id": "p1", "source": 0, "dest": 5}]},
    )

    response = client.put(f"/simulations/{run_id}/config", json=_CONFIG_PAYLOAD)
...
def test_submit_requests_twice_returns_409(tmp_path: Path) -> None:
    ...
    client.post(f"/simulations/{run_id}/requests", json=first_requests)

    response = client.post(f"/simulations/{run_id}/requests", json=first_requests)
