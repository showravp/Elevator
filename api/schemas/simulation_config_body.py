...
class SimulationConfigBody(BaseModel):
    """Used for both creating a run (POST /simulations) and updating its config
    (PUT /simulations/{id}/config) — same shape either way, only the guard on whether the
    update is currently allowed differs, and that lives in the route handler, not here."""
